(() => {
    ...
    const includeDeadline = document.getElementById('include-deadline');
    ...
    function presentation() {
        return { templateId: template.value, nickname: draft.nickname, color: color.value, bold: document.getElementById('content-bold').checked, card: document.getElementById('content-card').checked,
            includeDeadline: includeDeadline.checked, deadline: draft.deadline || '', includeSource: includeSource.checked,
    ...
    function presentationState() {
        ...
        const hasDeadline = MessageModel.templates.find(t => t.id === template.value)?.hasDeadline;
        document.getElementById('deadline-control').hidden = !hasDeadline;
        document.getElementById('deadline-fields').hidden = !hasDeadline || !includeDeadline.checked;
        for (const prop of ['bold', 'card']) document.getElementById(`content-${prop}`).checked = draft[prop] !== false;
    }
    function ddlDays() {
    ...
    function setupDeadline() {
        ...
        const initial = draft.deadline || `${now.getUTCFullYear()}-${pad(now.getUTCMonth() + 1)}-${pad(now.getUTCDate())}T23:59`;
        const [date, time] = initial.split('T'), [y, m, d] = date.split('-'), [h, min] = time.split(':');
        const months = Array.from({ length: 24 }, (_, i) => { const value = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + i, 1)); return [`${value.getUTCFullYear()}-${pad(value.getUTCMonth() + 1)}`, `${value.getUTCFullYear()} 年 ${value.getUTCMonth() + 1} 月`]; });
    ...
    function renderTemplateButtons() {
        ...
        MessageModel.templates.forEach(item => {
            ...
            button.type = 'button';
            button.className = 'btn-small ripple-container';
            button.dataset.njuRipple = '';
            ...
            globalThis.NjuRipple?.attach(button);
        });
    }
    function updateDays() {
    ...
    function preview() {
        ...
        previewSubject.textContent = subject.value || '未填写主题';
        previewSubject.title = subject.value || '';
    ...
    function saveDraft() {
        ...
        if (draft.formatVersion === 2) Object.assign(draft, {
            color: color.value, includeSource: includeSource.checked, includeDeadline: includeDeadline.checked,
            bold: document.getElementById('content-bold').checked, card: document.getElementById('content-card').checked
        });
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local' && Object.keys(changes).some(k => k.startsWith('resend_'))) updateEmail().catch(() => messageNotice('读取失败', '配置读取失败。', true));
    });
    ...
    template.addEventListener('change', async () => {
        ...
        if ((subject.value !== original.subject || text.value !== original.text) && !await messageConfirm({
            title: '切换消息模板', message: '切换模板将替换当前标题和正文。', confirmText: '继续切换'
        })) { template.value = previous; syncTemplateButtons(); return; }
    ...
    for (const prop of ['bold', 'card']) document.getElementById(`content-${prop}`).addEventListener('change', event => {
        if (!draft || busy) return;
        draft[prop] = event.target.checked; preview(); saveDraft().catch(saveError);
    });
    [color, includeSource].forEach(input => input.addEventListener('change', () => { if (draft && !busy) { preview(); saveDraft().catch(saveError); } }));
    includeDeadline.addEventListener('change', () => {
        if (!draft || busy) return;
        draft.includeDeadline = includeDeadline.checked;
        if (includeDeadline.checked && !draft.deadline) draft.deadline = `${ddlMonth.value}-${pad(ddlDay.value)}T${pad(ddlHour.value)}:${pad(ddlMinute.value)}`;
        presentationState(); preview(); saveDraft().catch(saveError);
    });
    ...
    form.addEventListener('submit', async event => {
        ...
        busy = true; fields.disabled = true; send.disabled = true;
        try {
            ...
            send.textContent = '已提交';
            document.getElementById('draft-status').textContent = '已提交，草稿已结束';
            let cleanupFailed = false;
            try { await chrome.storage.local.remove(key); }
            catch (_) { cleanupFailed = true; await messageNotice('邮件已提交', messageResult(result) + '\n本地草稿清理失败，请勿重复发送。', true); }
            if (!cleanupFailed) await messageNotice('邮件已提交', messageResult(result));
        } catch (error) {
            messageStatus(error.message, true);
            await messageNotice('发送失败', error.message, true);
            const data = await chrome.storage.local.get(`NJU_MESSAGE_ATTEMPT_${id}`).catch(() => ({}));
            receipt = data[`NJU_MESSAGE_ATTEMPT_${id}`];
        }
        finally { if (!submitted) { fields.disabled = false; send.disabled = false; busy = false; } }
    });
    (async () => {
        if (!await messageConsent()) { location.href = '../options/options.html'; return; }
        await updateEmail();
        ...
        receipt = data[`NJU_MESSAGE_ATTEMPT_${id}`];
        if (receipt?.result) { messageStatus(messageResult(receipt.result)); await messageNotice('邮件已提交', messageResult(receipt.result)); return; }
        draft = data[key];
        ...
        includeDeadline.checked = Boolean(draft.includeDeadline && draft.deadline);
        ...
        fields.disabled = false; send.disabled = false;
        if (receipt) { const warning = '上次发送状态未确认，请保持原内容重试，或先到 Resend 控制台核实。'; messageStatus(warning, true); await messageNotice('发送状态未确认', warning, true); }
    })().catch(error => { messageStatus(error.message, true); messageNotice('无法打开草稿', error.message, true); });
})();
