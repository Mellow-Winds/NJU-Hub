...
### 📊 GPA 快速查询
- 点击插件图标一键跳转至精英系统（elite.nju.edu.cn）查看 GPA
- 无需手动导航多级菜单

### 📚 选课助手
- **红黑榜分析**：基于专业背景与个人偏好，AI 深度分析课程评价，标注高绩点"红榜"与避坑"黑榜"
- **冲突预警**：自动检测时间冲突课程
- **置顶收藏**：将感兴趣的课程固定在选课列表顶部
- **校区筛选**：按仙林/鼓楼/浦口/苏州校区过滤
- **SeaTable 云端同步**：对接学长维护的公共评价库，一键同步课程评价数据
- 支持导入/导出 Excel 评价数据，批量管理课程评价
- AI 配置独立，可选用不同 API Key 与模型

### 🎓 自动评教
- 全自动状态机评教：锁定"很好"选项，随机生成合理评语
- 自动处理弹窗，期末/助教评价自动流转
- 内置语料库，无需额外 AI 配置

### 🎬 智汇南雍（LMS）增强
- **解除视频限制**：解除 LMS 视频播放的倍速/拖拽限制
- **自动连播**：视频播放完毕自动跳转下一集
- **课件批量下载**：下载列表默认全选 + 文件选择框
- **视频画中画**：支持视频浮窗播放

### 🖥️ SEEC 门户增强
- 软件工程 SEEC 门户作业三栏大面板，一览所有作业信息
...
> Edge 对开发者模式插件的提示较为温和，推荐作为日常使用浏览器。
