...
- 作业、考试/测验、课程/活动及待办模板提供“附上 DDL”开关；开启后按 UTC+8 选择截止日期。DDL 仅写入正文，不改变发送时间。样式、称呼、DDL 及来源开关随草稿保存。旧版已编辑正文或状态未确认的发送请求保留原内容和格式，避免误改及重复发送。
- API Key 仅持久化于 `chrome.storage.local`，不同步到浏览器账号，不经过 NJU-Hub 中转；**调用 API 时必须直接向 Resend 发送密钥用于鉴权**。本地存储不等于加密存储，请使用专用发送密钥，不要分享密钥。
- 只有点击发送/测试发送才会向 Resend 发送邮件内容（含称呼和 DDL）和本人邮箱。HTTP(S) 来源 URL 随草稿保存在本地；“附上来源链接”默认关闭，开启后才在邮件中加入“点击跳转来源”。请检查链接是否包含私密参数。
- 隐私声明中的“暂不使用”会立即暂停消息订阅；本地配置和草稿不会被删除，重新阅读并同意后即可恢复。暂停状态下后台会拒绝发送和测试请求。
