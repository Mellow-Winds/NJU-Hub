...
1. 设置页 → **其他功能 → 消息订阅设置**，填写 Resend API Key 和 **Resend 注册邮箱**。
2. 点击“保存并发送测试邮件”，在自己的邮箱确认收信（也请检查垃圾邮件）。
3. 在网页选中文字，右键选择“加入 NJU-Hub 消息订阅”，编辑主题、纯文本正文和时间后提交。

- 发件地址固定为 `onboarding@resend.dev`，无需验证域名；可修改显示名，不能修改发件地址。Resend 共享测试域名仅允许发送到账号注册邮箱，邮箱归属由 Resend 校验。
- 只支持自己发给自己：编辑页邮箱只读，后台忽略外部传入的收件地址。当前不支持模板、他人收件、群发、抄送、附件或收件箱。未来如支持发给他人，才需要验证域名并修改 `from`。
- API Key 仅持久化于 `chrome.storage.local`，不同步到浏览器账号，不经过 NJU-Hub 中转；**调用 API 时必须直接向 Resend 发送密钥用于鉴权**。本地存储不等于加密存储，请使用专用发送密钥，不要分享密钥。
- 只有点击发送/测试发送才会向 Resend 发送邮件内容和本人邮箱。页面来源标题仅留在本地，来源 URL 不采集、不附加到邮件。
- 发送时间留空表示立即发送；预约按本机时区解释并转换成 UTC，最多提前 30 天。提交后由 Resend 执行，关闭浏览器或卸载插件不会取消预约，可在 [Resend 控制台](https://resend.com/emails) 管理。
